...
#       compared to the parametricbootstrap
    ...
    inflation = map(zip(m.reterms, blups)) do (trm, re)
        ...
        λemp = cholesky(cov(re'; corrected=false); check=false).L    # L_S in CGR
...
"""
    unit_inflation(m::LinearMixedModel, blups=ranef(m), resids=residuals(m)))
...
See also [`inflation_factor`](@ref).
"""
function unit_inflation(m::LinearMixedModel, blups=ranef(m), resids=residuals(m))
    return repeat([1.0], length(m.reterms)+1)
end
